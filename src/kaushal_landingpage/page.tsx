...
export default function Home() {
    ...
    <main className='flex min-h-screen h-fit flex-col items-center justify-center relative'>
      ...
      <header id="home" className="flex flex-col-reverse md:flex-row w-full h-screen max-w-7xl items-center justify-center p-8 relative overflow-x-hidden">
        <div className='w-full h-2/4 md:h-full md:w-2/5 flex flex-col justify-center items-center md:items-start gap-8'>
          ...
          <p className='max-w-md text-sm md:text-base text-zinc-500'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Eveniet optio aliquid repudiandae eligendi earum hic consectetur! Voluptatibus, eum dignissimos ea a</p>
          <div className='w-full flex items-center justify-center md:justify-start gap-4'>
            <button className='w-48 h-12 text-sm sm:text-base rounded bg-white text-black hover:bg-green-700 hover:text-white transition-colors'>Explore!</button>
            <button className='w-48 h-12 text-sm sm:text-base rounded hover:bg-white hover:text-white hover:bg-opacity-5 transition-colors'>Connect</button>
          </div>
        </div>

        <div className='w-full h-4/5 md:h-full md:w-5/6 flex items-center justify-center relative -z-10'>
          <Spline className="" scene='https://prod.spline.design/eQKMhU-5qKLyMUMX/scene.splinecode'/>
      ...
      <section id="about" className="h-fit min-h-screen w-full flex relative items-center justify-center p-8">
        ...
        <div className="w-full h-full flex items-center justify-center flex-col gap-8 max-w-7xl">
          <h3 className='text-4xl md:text-5xl font-bold'> Lorem ipsum dolor sit </h3>
  ...
  <div className="relative z-10 text-white p-8 text-center">
    <h2 className="text-2xl md:text-4xl font-bold mb-4">Footer Title</h2>
    <p className="text-lg md:text-xl z-10">Footer content goes here. Add links or any other information.</p>
  </div>
</footer>
...
function Navbar() {
    return (
      <div className=' fixed top-3  w-11/12 h-16 backdrop-filter backdrop-blur-xl bg-white bg-opacity-20 rounded-xl border-gray-200 flex items-center justify-center z-50'>
        <div className='max-w-7xl w-full flex items-center justify-between p-4'>
          <h6 className='font-bold'>Avocodos</h6>
          <ul className='flex gap-8'>
            <li><Link className='hover:text-green-500 transition-colors text-xs sm:text-base' href="#home">Home</Link></li>
            <li><Link className='hover:text-green-500 transition-colors text-xs sm:text-base' href="#about">About</Link></li>
            <li><Link className='hover:text-green-500 transition-colors text-xs sm:text-base' href="#signup">Sign Up</Link></li>
          </ul>
        </div>
      </div>
    )
